...
* p21_taxrevResEx0(ttot,regi) = sum(pe2rlf(peEx(enty),rlf), (p21_tau_fuEx_sub(ttot,regi,enty) + cm_extractionTax * pm_taxCO2eq(ttot,regi) * pm_cintraw(enty)) * vm_fuExtr.l(ttot,regi,enty,rlf));
p21_taxrevResEx0(ttot,regi) = sum(pe2rlf(peEx(enty),rlf), (p21_tau_fuEx_sub(ttot,regi,enty) + cm_extractionTax * 1 * 0.5) * vm_fuExtr.l(ttot,regi,enty,rlf));
